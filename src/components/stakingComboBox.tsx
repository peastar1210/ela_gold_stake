            ...
            <Listbox.Options className="sm:text-sm absolute z-[9999] mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none">
              {props.period.map((period, periodIdx) => (
                ...
                >
                  {({ selected }) => (
                    <>
                      <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'} relative w-full`}>
                        {period.period}
                        {`(`}
                        {(period.multiply / 100).toFixed(2)}x{`)`}
                      </span>
                      {selected ? (
