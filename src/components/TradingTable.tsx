...
interface Column {
  id: 'TXN' | 'time' | 'amount' | 'address' | 'type' | 'priceEla' | 'priceUSD' | 'totalUSD' | 'from' | 'gold'
  label: string
  minWidth?: number
  align?: 'left'
...
export default function TradingTable(tableData: any) {
  ...
  const handleConvertRemainTime = date => {
  ...
  return (
    <Paper
      ...
      sx={{ width: '100%', overflow: 'hidden', borderRadius: 2, color: "black" }}
      className="mt-[10px]"
    >
      <TableContainer
        sx={{
      ...
      >
        ...
        >
          ...
          <TableBody>
            {tradingData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
              return (
                <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                  <TableCell align="left" className="text-nowrap">
                    <div className="text-black">{handleConvertRemainTime(row.attributes.block_timestamp)}</div>
                  </TableCell>
                  <TableCell align="left">
                    {row.attributes.kind === 'buy' ? (
                      <div className="rounded-full bg-green-200 text-center text-green-900">buy</div>
                    ) : (
                      <div className="rounded-full bg-red-200 text-center text-red-900">sell</div>
                    )}
                  </TableCell>
                  <TableCell align="left">
                    <div className="text-black">
                      {parseFloat(row.attributes.price_to_in_currency_token).toFixed(2)}
                    </div>
                  </TableCell>
                  <TableCell align="left">
                    <div className="text-black">{parseFloat(row.attributes.price_to_in_usd).toFixed(2)}</div>
                  </TableCell>
                  <TableCell align="left">
                    {row.attributes.kind === 'buy' ? (
                      <>
                        <div className="text-black">
                          {parseFloat(parseFloat(row.attributes.to_token_amount).toFixed(6))}
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="text-black">
                          {parseFloat(parseFloat(row.attributes.from_token_amount).toFixed(6))}
                        </div>
                      </>
                    )}
                  </TableCell>
                  <TableCell align="left">
                    <div className="text-black">{parseFloat(row.attributes.volume_in_usd).toFixed(4)}</div>
                  </TableCell>
                  <TableCell align="left">
                    <Link href={`https://esc.elastos.io/address/${row.attributes.tx_from_address}`} target="_blank">
                      <div className="text-black">{'...' + row.attributes.tx_from_address.substring(36, 43)}</div>
                    </Link>
                  </TableCell>
                  <TableCell align="left">
                    <Link href={`https://esc.elastos.io/tx/${row.attributes.tx_hash}/token-transfers`} target="_blank">
